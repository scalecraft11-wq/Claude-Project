...
} from "@/lib/admin/actions/shipping";
import type { ShippingMethodInput } from "@/lib/validation/admin/shipping";

export function ShippingMethodRowActions({
  methodId,
  name,
  defaultValues,
}: {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    const result = await deleteShippingMethodAction(methodId);
    setIsDeleting(false);
    if (!result.success) {
  ...
  return (
    <>
      ...
      <Modal open={editOpen} onOpenChange={setEditOpen} title={`Edit ${name}`}>
        <ShippingMethodForm
          defaultValues={defaultValues}
          onSubmit={(values) => updateShippingMethodAction(methodId, values)}
